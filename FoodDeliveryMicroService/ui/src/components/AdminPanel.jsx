...
export default function AdminPanel() {
  ...
  const clearData = async () => {
    if (!window.confirm('Are you sure? This will delete ALL data from the database!')) return
            ...
            {loading ? (
              ...
              <div className="db-info">
                ...
                <div className="stats-grid">
                  <motion.div className="stat-box" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3 }}>
                    <div className="stat-icon restaurant-icon"></div>
                    <div className="stat-value">{dbStatus.collections.restaurants}</div>
                    <div className="stat-label">Restaurants</div>
                  </motion.div>

                  <motion.div className="stat-box" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.4 }}>
                    <div className="stat-icon menu-icon"></div>
                    <div className="stat-value">{dbStatus.collections.menuItems}</div>
                    <div className="stat-label">Menu Items</div>
                  </motion.div>

                  <motion.div className="stat-box" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.5 }}>
                    <div className="stat-icon order-icon"></div>
                    <div className="stat-value">{dbStatus.collections.orders}</div>
                    <div className="stat-label">Orders</div>
                  </motion.div>

                  <motion.div className="stat-box" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.6 }}>
